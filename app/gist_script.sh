...
pip3 install fastapi uvicorn pynamodb
...
Adam,Keller,keladam@amazon.com,8888888888
EOF
...
cat << EOF >> /usr/local/user_api/models/dynamo_model.py
...
class UserModel(Model):
    ...
    class Meta:
        ...
        region = "us-west-2" #Grab from ec2 metadata endpoint
...
chmod +x /usr/local/user_api/models/dynamo_model.py
touch /usr/local/user_api/models/__init__.py
